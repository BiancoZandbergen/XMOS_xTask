 ...
 * the sender and the recipient tasks reside on different kernels             *
 * with the kernels connected to different Communication Servers on           *
 * different tiles. The messages are transferred over the ring bus.           * 
 *                                                                            *
 ******************************************************************************/
#include <stdio.h>
#include <xccompat.h>
...
void task_2(void *p)
{
  ...
  while(1) {
    buf = xtask_get_inbox(TASK2_MAILBOX, ITC_ANYWHERE);
    data = (unsigned int *) buf->data;
